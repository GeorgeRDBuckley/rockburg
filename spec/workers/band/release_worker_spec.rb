...
RSpec.describe Band::ReleaseWorker, type: :worker do
  ...
  let(:release) { create(:release, kind: :album, band: band) }
  let(:hours) { 1 }
  let(:activity) { create :activity, action: :release, starts_at: Time.current, ends_at: Time.current + hours * ENV["SECONDS_PER_GAME_HOUR"].to_i }
  ...
  subject do
    Sidekiq::Testing.inline! do
      described_class.perform_async band.to_global_id, release.to_global_id, activity.id
    end
  end

  it 'adds one happening' do
