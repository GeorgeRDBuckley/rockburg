...
RSpec.describe Band::RecordSingleWorker, type: :worker do
  ...
  subject do
    Sidekiq::Testing.inline! do
      described_class.perform_async band.to_global_id, recording.to_global_id, hours, activity.id
    end
  end

  it 'adds three happenings' do
    @happenings = band.happenings.count
    subject
    expect(band.happenings.count).to eq(@happenings + 3)
  end
