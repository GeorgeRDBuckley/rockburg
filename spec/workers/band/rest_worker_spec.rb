...
RSpec.describe Band::RestWorker, type: :worker do
  ...
  let(:hours) { 1 }
  let(:activity) { create :activity, action: :rest, starts_at: Time.current, ends_at: Time.current + hours * ENV["SECONDS_PER_GAME_HOUR"].to_i }
  ...
  it 'adds 0 happening' do
    @happenings = band.happenings.count
    subject
    expect(band.happenings.count).to eq(@happenings + 0)
  end
