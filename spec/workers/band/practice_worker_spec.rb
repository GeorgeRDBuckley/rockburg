...
RSpec.describe Band::PracticeWorker, type: :worker do
  ...
  subject do
    Sidekiq::Testing.inline! do
      described_class.perform_async band.to_global_id, hours, activity.id
    end
  end

  it 'adds four happenings' do
    @happenings = band.happenings.count
    subject
    expect(band.happenings.count).to eq(@happenings + 4)
  end
